...
serve(async (req) => {
  try {
    ...
    // 2. Buscar TODOS os utilizadores (tokens e user_ids)
    ...
    if (!records || records.length === 0) {
      return new Response(JSON.stringify({ status: "No users to notify" }), { status: 200 });
    }

    // 3. Inserir no histórico (Sininho) para todos
    // Dividimos em lotes de 100 para não estourar limites do Supabase
