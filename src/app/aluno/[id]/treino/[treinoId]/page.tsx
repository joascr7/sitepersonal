...
const CabecalhoRelogio = ({ nomeAluno }: { nomeAluno?: string }) => {
  ...
  return (
    <div className="flex flex-col">
      ...
      <h1 className="text-lg font-black text-[var(--text-primary)] tracking-tight leading-none mt-0.5">
        {nomeAluno?.split(' ')[0] || 'Aluno'}
      </h1>
    </div>
  );
};
  ...
  return (
    <main style={themeStyles} className="min-h-screen w-full bg-[var(--bg)] text-[var(--text-primary)] transition-colors duration-500 font-sans antialiased pt-[max(env(safe-area-inset-top),1.5rem)] pb-[env(safe-area-inset-bottom)] px-4 relative">
      ...
      <div className="max-w-2xl mx-auto pb-32">
        ...
        {/* SELETOR DE TREINO PREMIUM */}
        <div className="relative w-full mb-8 group">
          <select
            value={treinoId}
            onChange={(e) => router.push(`/aluno/${id}/treino/${e.target.value}`)}
            className="w-full bg-[var(--surface)] text-[var(--text-primary)] text-sm font-black py-5 px-6 rounded-[1.5rem] appearance-none border border-[var(--border)] outline-none tracking-wide uppercase cursor-pointer shadow-md transition-all focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/20 relative z-10"
          >
            {todasFichas.map((f) => (
          ...
          <div className="absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none text-[var(--text-secondary)] text-xs font-black z-20">
            <FaChevronDown />
          </div>
        </div>
        ...
        <div className="mb-10">
          <div className="flex justify-between items-center mb-3">
            <p className="text-[var(--primary)] font-bold text-[11px] uppercase tracking-[0.2em] flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[var(--primary)] animate-pulse shadow-[0_0_8px_var(--primary)]"></span>
              {t.totalSessions}: <span className="text-[var(--text-primary)]">{sessoesContador}</span>
            </p>
            {treinoIniciado && (
              <div className="flex items-center gap-2 bg-[var(--primary)] text-white px-4 py-2 rounded-xl shadow-md">
                <span className="text-[9px] font-black uppercase tracking-widest"><FaClock className="inline mr-1" size={10}/> Tempo:</span>
                <span className="font-mono text-sm font-black leading-none">{formatarTempoGeral(segundosTreino)}</span>
              </div>
            )}
          </div>
          
          <div className="w-full h-2.5 bg-[var(--surface-sec)] rounded-full overflow-hidden border border-[var(--border)] shadow-inner">
            <div className="h-full bg-gradient-to-r from-blue-400 to-indigo-600 transition-all duration-700 ease-out relative" style={{ width: `${progresso}%` }}>
              <div className="absolute top-0 right-0 bottom-0 w-10 bg-white/30 blur-sm" />
            </div>
          </div>
        </div>

        {/* CALL TO ACTION PREMIUM (Pronto para treinar?) */}
        {!treinoIniciado && (
           <div className="mb-10 text-center px-4 animate-in zoom-in-95">
             <h2 className="text-2xl font-black text-[var(--text-primary)] tracking-tight mb-2">{t.ready}</h2>
             <p className="text-[var(--text-secondary)] text-sm mb-6 font-medium">Inicie o cronômetro para começar a sua sessão e registrar seu tempo.</p>
             <button onClick={() => { setTreinoIniciado(true); setDataInicio(new Date()); setExpandedId(0); }} 
               className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-5 rounded-[1.2rem] font-black text-sm uppercase tracking-widest shadow-[0_8px_30px_rgb(79,70,229,0.3)] hover:shadow-[0_8px_30px_rgb(79,70,229,0.5)] transition-all active:scale-95 flex items-center justify-center gap-3">
               <FaPlay size={14} /> {t.startWorkout}
             </button>
           </div>
        )}
        
        {/* LISTA DE EXERCÍCIOS PREMIUM COM IMAGEM GRANDE */}
        <div className={`space-y-4 transition-all duration-500 ${treinoIniciado ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
          {exercicios.map((ex, exIndex) => {
            ...
            return (
              <div key={exIndex} className={`bg-[var(--surface)] rounded-[1.5rem] overflow-hidden transition-all duration-300 shadow-sm border ${isConcluido ? 'border-[var(--success)]/50 bg-[var(--success)]/5' : 'border-[var(--border)]'}`}>
                
                <button 
                  onClick={() => setExpandedId(isExpanded ? null : exIndex)}
                  className="w-full p-4 flex items-center justify-between hover:bg-[var(--surface-sec)]/50 transition-colors"
                >
                  ...
                  <div className="flex items-center gap-3 shrink-0 bg-[var(--surface-sec)] p-2.5 rounded-full">
                ...
                {isExpanded && (
                  <div className="p-5 border-t border-[var(--border)] bg-[var(--surface-sec)]/30">
                    
                    {ex.observacao && (
                      <div className="p-4 bg-[var(--primary)]/5 border-l-4 border-[var(--primary)] rounded-r-xl text-sm mb-5 shadow-sm">
                        <p className="text-[10px] font-black uppercase tracking-widest text-[var(--primary)] mb-1">Dica do Personal:</p>
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-5">
                      ...
                      <div className="bg-[var(--surface)] border border-[var(--border)] p-4 rounded-2xl flex flex-col items-center justify-center text-center shadow-sm">
                        <FaUndo className="text-[var(--primary)] text-sm mb-2" />
                        <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest">Repetições</span>
                        <span className="text-lg font-black text-[var(--text-primary)] mt-1">
                          {repsExibicao}
                        </span>
                      </div>

                      <div className="bg-[var(--surface)] border border-[var(--border)] p-4 rounded-2xl flex flex-col items-center justify-center text-center shadow-sm focus-within:border-[var(--primary)] focus-within:ring-2 focus-within:ring-[var(--primary)]/20 transition-all relative group">
                        <FaDumbbell className="text-[var(--primary)] text-sm mb-2" />
                        <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest">Carga</span>
                        
                        <div className="flex items-center justify-center mt-1 w-full relative pl-4">
                          <input 
                            type="number" 
                            ...
                            className="w-12 bg-transparent text-center text-lg font-black text-[var(--text-primary)] outline-none pr-1"
                            style={{ WebkitAppearance: 'none', margin: 0 }}
                          />
                          
                          <FaPencilAlt size={10} className="text-[var(--primary)] opacity-0 group-hover:opacity-100 transition-opacity mr-1.5 shrink-0" />
                          ...
                          <select 
                            ...
                            className="bg-transparent text-[10px] font-black text-[var(--text-secondary)] uppercase outline-none cursor-pointer appearance-none shrink-0"
                          >
                      ...
                      <button 
                        ...
                        className="bg-[var(--surface)] border border-[var(--border)] p-4 rounded-2xl flex flex-col items-center justify-center text-center shadow-sm active:scale-95 transition-all hover:border-[var(--primary)]"
                      >
                        <FaStopwatch className="text-[var(--primary)] text-sm mb-2" />
                        <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest">Intervalo</span>
                        <span className="text-lg font-black text-[var(--text-primary)] mt-1">
                          {intervaloExibicao}
                        </span>
                      </button>
                    ...
                    <button 
                      ...
                      className={`w-full py-4 rounded-[1rem] font-black text-xs uppercase tracking-widest transition-all active:scale-[0.98] flex items-center justify-center gap-2 ${
                        isConcluido 
                          ? 'bg-[var(--surface)] text-[var(--text-secondary)] border border-[var(--border)] shadow-sm hover:bg-[var(--surface-sec)]' 
                          : 'bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-indigo-600/30 hover:shadow-indigo-600/50'
                      }`}
                    >
                      {isConcluido ? <><FaUndo size={12} /> {t.undoEx}</> : <><FaCheck size={14} /> {t.concludeEx}</>}
                    </button>
          ...
          <div className="mt-10 animate-in fade-in slide-in-from-bottom-4">
            <button 
              ...
              className={`w-full py-5 rounded-[1.2rem] font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-3 shadow-xl ${
                todosFinalizados 
                  ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white hover:brightness-110 hover:shadow-emerald-500/40 active:scale-95' 
                  : 'bg-[var(--surface-sec)] text-[var(--text-secondary)] cursor-not-allowed opacity-70 border border-[var(--border)]'
              }`}
            >
