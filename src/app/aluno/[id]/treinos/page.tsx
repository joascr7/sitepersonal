      ...
      if (data) {
        const processadas = data.map(f => {
          ...
          try {
            ...
            exercicios = f.descricao ? f.descricao.split('\n\n').map((bloco: string) => {
              const linhas = bloco.split('\n');
              const header = linhas[0].split(' | ');
              return {
                nome: header[0]?.replace('EXERCÍCIO: ', '') || 'Treino',
                video_url: '',
                series: linhas.slice(1).map(s => {
                  const [r, c, i] = s.split(' | ');
                  return { reps: r?.replace('Rep: ', '') || '', carga: c?.replace('Carga: ', '') || '', intervalo: i?.replace('Int: ', '') || '0' };
                })
              };
            }) : [];
          }
    ...
    <main className="min-h-screen bg-gray-50 p-6 md:p-12">
      <div className="max-w-xl mx-auto">
        ...
        {fichas.length === 0 ? (
          ...
          fichas.map((f) => (
            <div key={f.id} className="bg-white p-8 rounded-3xl mb-4 flex justify-between items-center shadow-sm border border-gray-100 hover:border-gray-200 transition-all">
              <div>
                <p className="text-lg font-black text-gray-900">{f.nome_treino}</p>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">{f.count} exercícios</p>
              </div>
              <button 
                onClick={() => router.push(`/aluno/${id}/treino/${f.id}`)}
                className="bg-gray-900 hover:bg-black text-white px-8 py-3 rounded-xl text-sm font-bold shadow-sm transition-all active:scale-[0.98]"
              >
                Abrir
              </button>
            </div>
